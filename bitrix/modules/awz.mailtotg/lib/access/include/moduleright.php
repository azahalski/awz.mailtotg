...
use Awz\Mailtotg\Access\Custom\ComponentConfig;
                ...
                window.awz_right_dialog = {
                    ...
                    reloadItems: function(items, event){
                        ...
                        items.forEach(function(itm){
                            if(itm.getEntityId()==='user' || itm.getEntityType()==='awzmailtotg-user'){
                                let itmEv = {
                                    type: 'users',
                                    id: 'U'+itm.getId(),
                                    key: 'U'+itm.getId(),
                            ...
                            }else if(itm.getEntityType()==='awzmailtotg-group'){
                                let itmEv = {
                                    type: 'groups',
                                    id: 'G'+itm.getId(),
                                    key: 'G'+itm.getId(),
                    ...
                    load: function(groupKey, options){
                        ...
                        if(groupData.hasOwnProperty('members')){
                            for(k in groupData['members']){
                                ...
                                if(itm.type==='users'){
                                    preselectedItems.push(['awzmailtotg-user',itm.id]);
                                }
                                else if(itm.type==='awzmailtotg-user'){
                                    preselectedItems.push(['awzmailtotg-user',itm.id]);
                                }
                                else if(itm.type==='awzmailtotg-group'){
                                    preselectedItems.push(['awzmailtotg-group',itm.id]);
                                }
